  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button className=" cursor-pointer size-9 relative overflow-hidden bg-[#ABABAD] hover:bg-[#ABABAD]/80 text-slate-800 font-semibold tex-xl">
          {workspaceLoading ? (
            <Loader className="size-5 animate-spin shrink-9" />
          ) : (
      ...
      <DropdownMenuContent side="bottom" align="start">
        ...
        {filteredWorkspace?.map((workspace) => (
          <DropdownMenuItem
            key={workspace._id}
            className="cursor-pointer capitalize overflow-hidden w-60"
